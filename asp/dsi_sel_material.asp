...
<title>::<%=getTermo(global_idioma, 175, "Material", 0)%></title>
...
<script type="text/javascript">
	...
	function pega_selecao() {
		...
		var sURL = "dsi_sel_material.asp?acao=alterar&usuario=<%=Session("codigo_usuario")%>&selecao=" + sSel + '&iBanner=<%=global_tipo_banner%>&iIdioma=<%=global_idioma%>';
...
<% 
	...
	if request.querystring("acao") = "alterar" then	
		...
		ROService.IncluiDadosDSI codUsu, 0, sSel
		...
		<td class="td_center_top">
            <div style="height: 420px; max-height: 420px; overflow: auto;">
				...
				<% 
                    ...
                    'Busca registros
                    Set ROService = ROServer.CreateService("Web_Consulta")	
                    sXMLSel = ROService.DadosTabela(roTAB_DSI_MATERIAL, Session("codigo_usuario"), 0,repositorio_institucional)
                    Set ROService = nothing
                    TrataErros(1)
					...
					if (left(sXMLSel,5) = "<?xml") then
						...
						if xmlRoot.attributes.getNamedItem("QUANTIDADE").value <> "0" then
							...
							Response.Write getTermo(global_idioma, 3926, "Não há dados cadastrados.", 0)
						end if
