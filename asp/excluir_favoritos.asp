...
<%
	...
	codigoTitulo = IntQueryString("codigoTitulo", 0)
	
	if IsNumeric(codigoTitulo) and codigoTitulo > 0 then
		Set ROService = ROServer.CreateService("Web_Consulta")
		Set Msg = ROService.ExcluirTituloFavorito(codigoTitulo, codigoFavorito, idioma)
		...
		if Msg.sMsg <> "" then
			...
			Response.Write "<input type='button' onclick=""parent.fechaPopup('asp');parent.atualiza_Lista_Favoritos("&iIndexSrv&", "&codigoFavorito&");"" value='"&getTermo(idioma, 220, "Fechar", 0)&"' />"
		else
			Response.Write "<script type='text/javascript'>parent.fechaPopup('asp');parent.atualiza_Lista_Favoritos("&iIndexSrv&", "&codigoFavorito&");</script>"
		end if
	elseif IsNumeric(codigoFavorito) then
		Set ROService = ROServer.CreateService("Web_Consulta")
		Set Msg = ROService.ExcluirFavorito(codigoFavorito, idioma)
		Set ROService = nothing
		TrataErros(1)
		%></div><br /><%
		if Msg.sMsg <> "" then
