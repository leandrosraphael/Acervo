...
'--------------------------------------------------------------------------------
sFichaStream = "<table class='tabela_grid_sels' style='border-spacing: 0; padding: 1px'>"
	...
	if xmlRoot.nodeName = "Pagina" then
		For Each xmlFicha In xmlRoot.childNodes
			if xmlFicha.nodeName  = "Ficha" then
				...
				sFichaStream = sFichaStream & "<tr>"
				'//-------------------------------------------------------------//
				'---------------------CELULA COM A FICHA------------------------//
				'//-------------------------------------------------------------//				
				sFichaStream = sFichaStream & "<td class='td_center_middle td_grid_ficha_marg'>"
				ficha = ""
				codigo_atual = 0
				tem_chamada = false
				...
				For Each xmlCampos In xmlFicha.childNodes
					...
					if cStr(Tipo) = "0" then
						...
						if xmlCampos.nodeName = "ASSUNTOS" then
							...
							ficha = ficha & "<tr>"
							ficha = ficha & "<td class='td_imp_esq esquerda'>"&desc_assunto&"</td>"
							For Each xmlSubCampos In xmlCampos.childNodes
								'*************************************
						...
						if xmlCampos.nodeName = "SITE" then
							site = xmlCampos.attributes.getNamedItem("Valor").value
							site_desc = xmlCampos.attributes.getNamedItem("Descricao").value
							ficha = ficha & "<tr>"
							ficha = ficha & "<td class='td_imp_esq esquerda'>"
							ficha = ficha & site_desc
							ficha = ficha & "</td><td class='td_imp_dir esquerda'>" & site & "</td></tr>"
						end if
					'*************************************
					'FICHA DE OBRAS
					'*************************************
					...
					elseif cStr(Tipo) = "3" then
						...
						if xmlCampos.nodeName = "ENT_PRINC" then
							...
							ficha = ficha & "<tr>"
							ficha = ficha & "<td colspan=2 class='td_imp_esq esquerda'>"
							ficha = ficha & autor_desc
							ficha = ficha & "</td><td class='td_imp_dir esquerda'>"&desc_autor&"</td></tr>"							
						end if
						'*************************************
				...
				sFichaStream = sFichaStream & "<br /><table style='border-spacing: 2px; padding: 1px; background-color: #FFFFFF; width: 98%'>"&ficha&"</table><br />"
				sFichaStream = sFichaStream & "</td></tr>"
			end if
			sequencial = sequencial + 1
		Next
	End if	
...
sFichaStream = sFichaStream & "</table>"
%>