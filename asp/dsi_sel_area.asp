...
<title>::<%=getTermo(global_idioma, 705, "Área", 0)%></title>
...
<script type="text/javascript">
	...
	function pega_selecao() {
		...
		var sURL = "dsi_sel_area.asp?acao=alterar&usuario=<%=Session("codigo_usuario")%>&selecao=" + sSel + '&iBanner=<%=global_tipo_banner%>&iIdioma=<%=global_idioma%>';
...
<% 
	...
	if request.querystring("acao") = "alterar" then	
		...
		ROService.IncluiDadosDSI codUsu, 2, sSel
		...
		<td class="td_center_top">
            <div style="height: 420px; max-height: 420px; overflow: auto;">
				...
				<% 
                    ...
                    'Busca registros
                    Set ROService = ROServer.CreateService("Web_Consulta")	
                    sXMLSel = ROService.DadosTabela(roTAB_DSI_AREA, Session("codigo_usuario"), 0,repositorio_institucional)
                    Set ROService = nothing
                    TrataErros(1)
